...
export function TeacherDashboardPage() {
  ...
  return (
    <AppShell title="Tableau de bord" active="prof-espace">
      ...
      <Card className="border-primary bg-primary text-primary-foreground">
        <CardContent className="pt-4">
          <div className="text-xs text-white/80">Revenus du mois</div>
          <div className="font-display text-4xl font-black">{fcfa(data.revenue)} F</div>
          <p className="text-sm text-white/85">{data.trend}</p>
        </CardContent>
      </Card>
      <Card>
      ...
      {data.pendingRequests ? (
        <Card>
          <CardContent className="pt-4">
            <Link to="/prof-offres" className="font-semibold">
              {data.pendingRequests} demande(s) en attente ›
            </Link>
          </CardContent>
        </Card>
      ) : null}
      {data.profileCompletion && !data.profileCompletion.complete ? (
        <MenuRow to="/prof-profil" label={`Compléter mon profil (${data.profileCompletion.percent}%)`} />
      ) : null}
    </AppShell>
  );
}
...
export function TeacherRequestsPage() {
  ...
  return (
    <AppShell title="Demandes" active="prof-offres">
      {offline ? <OfflineBanner onRetry={() => void reload()} /> : null}
...
export function TeacherEarningsPage() {
  ...
  return (
    <AppShell title="Revenus" active="prof-revenus">
      {offline ? <OfflineBanner onRetry={() => void reload()} /> : null}
      <Card>
        <CardContent className="pt-4">
  ...
  return (
    <AppShell title="Mon profil prof" back="/compte" active="compte">
      ...
      >
        ...
        <div className="space-y-1.5">
          <Label>Programmes (séparés par virgule)</Label>
          <Input name="programs" defaultValue={(data?.programs || []).join(", ")} key={(data?.programs || []).join()} />
        </div>
        <div className="space-y-1.5">
          <Label>Niveaux (séparés par virgule)</Label>
